...
void get_subscribe_topics(topic_t** dest, size_t* size, const enum ROLES_T role){
    switch(role){
      case ROLE_0:
        *size = 57;
        *dest = (topic_t*) malloc(sizeof(topic_t) * 57);
        ...
        (*dest)[8] = build_topic_data_simulator("+", "+");
        (*dest)[9] = build_topic_data_gps("+", "+");
        (*dest)[10] = build_topic_data_brusa("+", "+");
        (*dest)[11] = build_topic_status("+", "+");
        (*dest)[12] = build_topic_status_info("+", "+");
        (*dest)[13] = build_topic_status_error("+", "+");
        (*dest)[14] = build_topic_status_alert("+", "+");
        (*dest)[15] = build_topic_status_can_frequencies("+", "+");
        (*dest)[16] = build_topic_status_lap_counter_status("+", "+");
        (*dest)[17] = build_topic_status_lap_counter_laps("+", "+");
        (*dest)[18] = build_topic_command("+", "+");
        (*dest)[19] = build_topic_command_send("+", "+");
        (*dest)[20] = build_topic_command_result("+", "+");
        (*dest)[21] = build_topic_file_transaction_request("+", "+");
        (*dest)[22] = build_topic_file_transaction_response("+", "+");
        (*dest)[23] = build_topic_file_transaction("+", "+", "+");
        (*dest)[24] = build_topic_file_transaction_begin("+", "+", "+");
        (*dest)[25] = build_topic_file_transaction_end("+", "+", "+");
        (*dest)[26] = build_topic_file_transaction_chunk("+", "+", "+");
        (*dest)[27] = build_topic_file_transaction_chunk_ack("+", "+", "+");
        (*dest)[28] = build_topic_action("+", "+");
        (*dest)[29] = build_topic_action_telemetry_config("+", "+");
        (*dest)[30] = build_topic_action_telemetry_config_set("+", "+");
        (*dest)[31] = build_topic_action_telemetry_config_get("+", "+");
        (*dest)[32] = build_topic_action_telemetry_config_content("+", "+");
        (*dest)[33] = build_topic_action_session_config("+", "+");
        (*dest)[34] = build_topic_action_session_config_set("+", "+");
        (*dest)[35] = build_topic_action_session_config_get("+", "+");
        (*dest)[36] = build_topic_action_session_config_content("+", "+");
        (*dest)[37] = build_topic_action_car_config("+", "+");
        (*dest)[38] = build_topic_action_car_config_set("+", "+");
        (*dest)[39] = build_topic_action_car_config_get("+", "+");
        (*dest)[40] = build_topic_action_car_config_content("+", "+");
        (*dest)[41] = build_topic_action_handcart_settings("+", "+");
        (*dest)[42] = build_topic_action_handcart_settings_set("+", "+");
        (*dest)[43] = build_topic_action_handcart_settings_get("+", "+");
        (*dest)[44] = build_topic_action_handcart_settings_content("+", "+");
        (*dest)[45] = build_topic_action_kill("+", "+");
        (*dest)[46] = build_topic_action_start("+", "+");
        (*dest)[47] = build_topic_action_reset("+", "+");
        (*dest)[48] = build_topic_action_stop("+", "+");
        (*dest)[49] = build_topic_action_precharge("+", "+");
        (*dest)[50] = build_topic_action_balance("+", "+");
        (*dest)[51] = build_topic_action_stop_balance("+", "+");
        (*dest)[52] = build_topic_action_charge("+", "+");
        (*dest)[53] = build_topic_action_stop_charge("+", "+");
        (*dest)[54] = build_topic_action_raw("+", "+");
        (*dest)[55] = build_topic_action_reset_lap_counter("+", "+");
        (*dest)[56] = build_topic_action_set_lap_counter_status("+", "+");
        break;
      case ROLE_1:
        *size = 57;
        *dest = (topic_t*) malloc(sizeof(topic_t) * 57);
...
topic_t build_topic_data_simulator(const char* vehicleId, const char* deviceId) {
    topic_t topic = {
		.qos = 0,
		.retain = false
	};
    snprintf(topic.topic, TOPIC_MAX_STR_LEN, "%s/%s/data/simulator", vehicleId, deviceId);
    return topic;
}
